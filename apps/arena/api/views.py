...
class ArenaListView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        point = Point(float(longitude), float(latitude), srid=4326) if latitude and longitude else None
        ...
        arenas = Arena.objects.exclude(mode_start__lte=current_time.time(), mode_end__gte=current_time.time())
        if start_time and end_time:
            arenas = Arena.objects.exclude(bron__start_time__lte=start_time, bron__end_time__gte=end_time,
                                           mode_start__lte=start_time.time(), mode_end__gte=end_time.time())
        if point:
            arenas = arenas.annotate(
                distance=((F('latitude') - float(latitude)) ** 2 + (F('longitude') - float(longitude)) ** 2) ** 0.5
            ).order_by('distance') #location field qilish kerak
        if show_more:
